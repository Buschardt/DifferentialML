# -*- coding: utf-8 -*-
"""
Created on Tue May  3 10:56:57 2022
...
def LSM_train_poly(St, Et,discount,s0,r):
    ...
    for i in range(n_excerises-1)[::-1]:
        cashflow = cashflow*discount
        X = St[:, i]#.copy()
        print(np.sqrt(np.exp((np.log(s0)+r*(i)/49)*2+(((np.sqrt((i)/49))*0.2)**2))*(np.exp(((np.sqrt((i)/49))*0.2)**2)-1)))
        std_=np.sqrt(np.exp((np.log(s0)+r*(i)/49)*2+(((np.sqrt((i)/49))*0.2)**2))*(np.exp(((np.sqrt((i)/49))*0.2)**2)-1))
        exercise = Et[:,i]#.copy()
        print(sum((exercise>0)))
        itm = (exercise>0)*(X>X.mean()-2*std_)
        print(sum(X<X.mean()-2*std_))
        print(sum(itm))
        try:
            continuationValue = lstsquares(X[itm], cashflow[itm], X,degree=5)
            boundary = max(X[itm][(continuationValue[itm]<exercise[itm])])
...
tp,cont,boundaries = LSM_train_poly(St, Et, discount,K,rMean)
