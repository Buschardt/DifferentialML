...
def standardLSM(S0, K,sigma,r,T,dt,discount,nPaths,dW = None,anti = False):
    ...
    print((cashflow).mean())
    print(deltas.mean())
    return ((cashflow).mean(),deltas.mean())#contValues#(cashflow).mean(),

def LSM_train_poly(St, Et,discount,K,anti):
...
tp,cont,boundaries = LSM_train_poly(St, Et, discount,K,anti=antiFlg)
